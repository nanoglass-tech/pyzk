...
class ZK_helper(object):
    ...
    def test_ping(self):
        ...
        return subprocess.call(
            args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=need_sh
        ) == 0
...
class ZK(object):
    ...
    def __init__(
        ...
        password=0,
        force_udp=False,
        ommit_ping=False,  # (tetap dipertahankan untuk kompatibilitas)
        verbose=False,
        encoding="UTF-8",
    ):
        """
        ...
        :param omit_ping: check ip using ping before connect (env override: ZK_SKIP_PING=1/true/yes/on)
        ...
        # env override untuk skip ping saat jalan di systemd ter-hardening
        omit_env = os.getenv("ZK_SKIP_PING", "").lower() in ("1", "true", "yes", "on")
        self.ommit_ping = bool(ommit_ping or omit_env)
    ...
    def __create_tcp_top(self, packet):
        """with the complete packet set top header"""
        length = len(packet)
    ...
    def __create_header(self, command, command_string, session_id, reply_id):
        """Puts the parts that make up a packet together and packs them into a byte string"""
    ...
    def __create_checksum(self, p):
        """Calculates the checksum of the packet to be sent to the time clock (zkemsdk.c)"""
        l = len(p)
        checksum = 0
        while l > 1:
    ...
    def __reverse_hex(self, hex_str):
        # bugfix: gunakan // dan hindari overshadow builtin 'hex'
        data = ""
    ...
    def __decode_time(self, t):
        """Decode a timestamp retrieved from the timeclock (zkemsdk.c - DecodeTime)"""
        t = unpack("<I", t)[0]
        second = t % 60
        t = t // 60
    ...
    def set_user(self, uid=None, name="", privilege=0, password="", group_id="", user_id="", card=0):
        ...
        if self.user_packet_size == 28:  # zk6
            if not group_id:
                group_id = 0
            try:
                command_string = pack(
    ...
    # --- TCP chunk helpers (dipertahankan; belum direfaktor besar) ---
    def __recieve_tcp_data(self, data_recv, size):
        """ data_recv, raw tcp packet; must analyze tcp_length; return (data, broken_header) """
        data = []
    ...
    def get_attendance(self):
        ...
        # guard: hindari ZeroDivisionError
        record_size = (total_size // self.records) if self.records > 0 else 0
        if self.verbose:
        ...
        if record_size == 8:
            ...
            while len(attendance_data) >= 16:
                ...
                if not tuser:
                    ...
                    uid = tuser[0].uid if tuser else user_id
                else:
                    uid = tuser[0].uid
                timestamp = self.__decode_time(timestamp)
                attendances.append(Attendance(user_id, timestamp, status, punch, uid))
            ...
            # MB40-VL / keluarga visible-light (49-byte)
            if self.verbose:
                print("--- Handling 49-byte records ---")
            while len(attendance_data) >= 49:
                try:
                    uid_val, user_id_bytes, status_val, timestamp_bytes, punch_val, _extra = unpack(
                        "<H24sB4sB12s5x", attendance_data[:49]
                    )
                    user_id_str = (user_id_bytes.split(b"\x00")[0]).decode(self.encoding, errors="ignore").strip()
                    timestamp_dt = self.__decode_time(timestamp_bytes)

                    final_user_id = user_id_str if user_id_str and user_id_str != "0" else None
                    if not final_user_id:
                        tuser = list(filter(lambda x: x.uid == uid_val, users))
                        final_user_id = tuser[0].user_id if tuser else str(uid_val)
                        if self.verbose and not tuser:
                            print(f"[zk] WARN: UID {uid_val} not found in users; using UID as user_id")

                    attendances.append(Attendance(final_user_id, timestamp_dt, status_val, punch_val, uid_val))
                    attendance_data = attendance_data[49:]
                except Exception as e_unpack_49:
                    if self.verbose:
                        print(f"[zk] WARN 49-byte parse error: {e_unpack_49} hex={codecs.encode(attendance_data[:49], 'hex')}")
                    # loncat 1 record agar tidak infinite loop pada data korup
                    attendance_data = attendance_data[49:]

        else:
            # fallback 40-byte std
            while len(attendance_data) >= 40:
                ...
                timestamp = self.__decode_time(timestamp)
                attendances.append(Attendance(user_id, timestamp, status, punch, uid))
                attendance_data = attendance_data[record_size:]  # record_size bisa 40/… sesuai device
